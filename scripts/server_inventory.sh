...
echo "## RNA-seq inputs"
printf 'RNA_COUNTS_MATRIX=%s\n' "$(find_first "$RNASEQ_RESULTS_DIR" -path '*/050-quantification/counts_matrix.tsv')"
printf 'RNA_NORMALIZED_MATRIX=%s\n' "$(find_first "$RNASEQ_RESULTS_DIR" -path '*/050-quantification/tpm_matrix.tsv' -o -path '*/050-quantification/star_cpm_matrix.tsv')"
printf 'RNA_DEG_RESULTS=%s\n' "$(find_first "$RNASEQ_RESULTS_DIR" -path '*/060-deg-analysis/DEGs_annotated_results.tsv' -o -path '*/060-deg-analysis/DEGs_results.tsv')"
